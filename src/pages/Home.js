  ...
  const slides = [
    {
      ...
      description2: '투페이스는 당신 대신 대화를 시작해주는 AI 솔루션을 제공합니다.',
    ...
    },
    {
      image: Img02,
      text: '"친구 만들기가 어려운\n사람들을 위한 새로운 시작"',
      description1: '투페이스는 당신의 성격에 맞는 인연매칭과 공간접점을 제공하고,',
  ...
  return (
    <div className="flex min-h-screen justify-center items-center relative bg-gray-300 font-pretendard font-[200]">
      ...
      <div className="w-full max-w-[400px] flex flex-col justify-between items-center bg-[#4F64D1] h-screen p-8">
        ...
        {/* 슬라이드되는 중앙 부분 */}
        <div className="relative w-full flex-grow flex justify-center items-center overflow-hidden" style={{ marginBottom: '20px' }}>
          {slides.map((slide, index) => (
            ...
            >
              ...
              <p className="text-[13px] text-white text-center">{slide.description1}</p>
              {slide.description2 && (
                <p className="text-[13px] text-white text-center">{slide.description2}</p>
              )}
              {slide.description3 && (
                <p className="text-[13px] text-white text-center">{slide.description3}</p>
              )}
            </div>
          ))}
        </div>
